...
from kestrel.codegen.prefetch import do_prefetch_async
...
async def get(stmt, session):
    ...
    if "variablesource" in stmt:
        ...
        # rs: RetStruct
        rs = await session.data_source_manager.query_async(
        ...
        return_var_table = await do_prefetch_async(
